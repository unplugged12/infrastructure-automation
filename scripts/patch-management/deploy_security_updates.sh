...
create_snapshot() {
    ...
    if command -v lvcreate &>/dev/null; then
        local root_vg
        root_vg=$(lvs --noheadings -o vg_name,lv_name | grep -E 'root|lv_root' | awk '{print $1}' | head -n1)
        local root_lv
        root_lv=$(lvs --noheadings -o vg_name,lv_name | grep -E 'root|lv_root' | awk '{print $2}' | head -n1)
    ...
    case "$PKG_MANAGER" in
        apt)
            ...
            if [[ $SECURITY_ONLY -eq 1 ]]; then
                UPDATES=$(apt-get upgrade -s | grep -cE '^Inst.*security')
        ...
        yum|dnf)
            ...
            local update_list=""
            if [[ $SECURITY_ONLY -eq 1 ]]; then
                update_list="$(${PKG_MANAGER} updateinfo list security 2>/dev/null || true)"
                if [[ -n "$update_list" ]]; then
                    UPDATES=$(grep -cEv '^\s*$' <<< "$update_list")
                else
                    UPDATES=0
                fi
                log INFO "Found ${UPDATES} security updates available"
            else
                update_list="$(${PKG_MANAGER} check-update -q 2>/dev/null || true)"
                if [[ -n "$update_list" ]]; then
                    UPDATES=$(grep -cEv '^\s*$' <<< "$update_list")
                else
                    UPDATES=0
...
send_email_notification() {
    ...
    local body
    body=$(cat <<EOF
Security Update Report for ${HOSTNAME}
...
main() {
    ...
    if [[ $CREATE_SNAPSHOT -eq 1 ]] && [[ -f "/tmp/snapshot_info_${TIMESTAMP}.txt" ]]; then
        local snapshot_info=""
        snapshot_info=$(<"/tmp/snapshot_info_${TIMESTAMP}.txt")
